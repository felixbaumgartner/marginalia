...
export function Sidebar({ activeBooks, selectedBookId, onSelectBookId, onRemoveBook, onClose }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-surface-raised border-r border-border flex flex-col h-full shrink-0">
      ...
      <nav className="flex-1 p-3 space-y-1">
        ...
        <NavLink to="/notes" className={linkClass} onClick={handleNavClick}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
          </svg>
          Notes
        </NavLink>
        <NavLink to="/archive" className={linkClass} onClick={handleNavClick}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-3 border-t border-border overflow-y-auto max-h-60">
          ...
          <div className="space-y-1">
            {activeBooks.map(book => (
              <button
                key={book.id}
                onClick={() => { onSelectBookId(book.id); onClose?.(); }}
              ...
              >
                ...
                <span
                  onClick={(e) => {
                    e.stopPropagation();
                    if (window.confirm(`Remove "${book.title}" from reading list?`)) {
